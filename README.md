...
### 硬件配置要求
为了保证代码的顺利运行（包含数据生成、机器学习模型训练、深度学习 TabNet 训练以及 PPO 强化学习），建议满足以下电脑配置参数：
- **操作系统**: Windows 10/11, macOS, 或 Linux
- **处理器 (CPU)**: Intel Core i5 / AMD Ryzen 5 或以上级别（多核 CPU 有助于加快随机森林和强化学习环境交互的速度）
- **内存 (RAM)**: 最低 8 GB，推荐 **16 GB** 或以上（数据处理与模型训练对内存有一定要求）
- **显卡 (GPU)**: **非必须**。本项目中的 TabNet 和 PPO 模型在纯 CPU 环境下即可流畅运行（耗时约几分钟）。如果有 NVIDIA 独立显卡且配置了 CUDA，PyTorch 会自动调用 GPU 加速训练过程。
- **存储空间**: 至少 2 GB 的可用磁盘空间（用于安装 Python 虚拟环境及依赖库，并保存生成的模型文件与图表）
...
8. **strategy_evaluation_table_5_2.csv** (对应论文 表5-2)：动态授信策略表现对比表，包含三种策略（保守、规则、PPO）的平均累计奖励、利息收益、违约损失及净收益。
9. **stress_test_table_5_4.csv** (对应论文 表5-4)：压力测试结果对比表数据，包含“常规场景”与“压力场景”下的平均累计奖励、平均违约损失和净收益。可以直接使用 Excel 打开，或复制到 Word 论文中。
